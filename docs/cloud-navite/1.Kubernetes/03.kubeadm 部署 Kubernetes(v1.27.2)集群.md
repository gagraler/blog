...
主机名 | IP | 操作系统 | 
---|---|---|
k8s-control-plane-1 | 172.17.40.174 | CentOS 7.9.2009 |
k8s-work-1   | 172.17.40.175 | CentOS 7.9.2009 | 
k8s-work-2   | 172.17.40.176 | CentOS 7.9.2009 |
...
| 组件        | 推荐配置                | 注意事项               |
|-----------|---------------------|--------------------|
| MetalLB模式 | Layer2（测试环境）        | 单点故障风险             |
| BGP（生产环境） | 需要网络设备支持            |                    |
| IP地址池     | 预留专用IP段             | 避免与现有网络冲突          |
| 防火墙       | 开放7472/TCP/UDP（BGP） | Layer2模式需允许ARP/NDP |
